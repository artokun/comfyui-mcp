...
//
// update-all and deferred snapshot restores are handed to ComfyUI-Manager and
...
//   - SHARED queue counts never identify OUR update-all: unrelated pending
...
//     update-all runs on regardless — so none is sent (could-not-verify).
...
async function cancelUpdateAll(op: PanelPendingOp): Promise<PanelPendingCancelReport> {
  ...
  // server the update-all was queued on. Never a proven cancel from that.
  if (!op.base) return cancelUpdateAllUnbased(op);
  const base = op.base;

  // A reset is only worth sending — and only REPORTABLE as a cancel — when
  // OUR update-all's state is provable. Shared queue counts can never provide
  ...
  try {
    ...
    return {
      ...
      detail:
        ...
        `The queued update-all may still be pending; the warning stands.`,
    };
  }
 ...
 * filter, so OUR update-all can never be told apart from unrelated pending
...
async function cancelUpdateAllUnattributable(
  ...
  if (!counts) {
    return {
      ...
      detail:
        ...
        `sent. The queued update-all may still be pending; the warning stands.`,
    };
  }
  if (counts.pending === 0 && counts.inProgress === 0 && !counts.processing) {
    return finalize(op, {
      ...
      detail:
        ...
        `NOTHING left to cancel — the update-all already drained (or never ` +
  ...
  if (counts.pending === 0) {
    ...
    return {
      ...
      detail:
        ...
        `drops PENDING work). The update-all may STILL move the panel after ` +
        `this pin — check install_comfyui(action:'panel', panel_action:'status') once it settles.`,
      pendingBefore: 0,
      pendingAfter: 0,
  ...
  return {
    ...
    detail:
      ...
      `which exposes no task history — the queued update-all cannot be told ` +
      `apart from UNRELATED pending work, and a queue reset would be blind: it ` +
      `could wipe someone else's queued tasks while the update-all runs on ` +
...
/**
 * v4 without a recorded ui_id: the update-all's tasks can be narrowed to this
...
async function cancelUpdateAllNoUiId(
  ...
  if (!mine) {
    return {
      ...
      detail:
        ...
        `was sent. The queued update-all may still be pending; the warning stands.`,
    };
  }
  if (mine.pending === 0 && mine.inProgress === 0 && !mine.processing) {
    return finalize(op, {
      ...
      detail:
        ...
        `— the update-all already drained (or never queued), so there was ` +
  ...
  if (mine.inProgress > 0 || mine.processing) {
    ...
    return {
      ...
      markerCleared: false,
      detail:
        `cannot cancel — ${busy} on ${base} and the update-all's tasks may be ` +
        `among them; in-flight work cannot be cancelled (a queue reset only ` +
        `drops PENDING work). NO reset was sent. The update-all may STILL move ` +
  ...
  return {
    ...
    markerCleared: false,
    detail:
      `the marker recorded no ui_id, so the update-all cannot be told apart ` +
 ...
 * unreadable. update-all names each per-pack task `${uiId}_${packKey}`, and
...
async function cancelUpdateAllProvable(
  ...
  if (mine.inProgress > 0 || mine.processing) {
    ...
    return {
      ...
      detail:
        ...
        `was sent. The update-all may STILL move the panel after this pin.`,
      pendingBefore: mine.pending,
      inProgress: mine.inProgress,
    };
  }
  if (mine.pending === 0) {
    return finalize(op, {
      ...
      detail:
        ...
        `have been moved — check install_comfyui(action:'panel', panel_action:'status').`,
      pendingBefore: 0,
      pendingAfter: 0,
      inProgress: 0,
    });
  ...
  // does not prove which pending queue entry belongs to this update-all. The
  ...
  return {
    ...
    detail:
      ...
      `work and cannot prove it cancelled this update-all, so NOTHING was sent; ` +
      `the warning stands.`,
    pendingBefore: mine.pending,
    inProgress: mine.inProgress,
  };
 ...
 * proves NOTHING about the server the update-all was queued on — and a reset
...
async function cancelUpdateAllUnbased(op: PanelPendingOp): Promise<PanelPendingCancelReport> {
  ...
  if (!counts) {
    return {
      ...
      detail:
        ...
        `${base} could not be read — the update-all may be queued on a ` +
  ...
  if (counts.pending === 0) {
    ...
    return {
      ...
      detail:
        ...
        `which proves NOTHING about the ComfyUI the update-all was actually ` +
  ...
  return {
    ...
    detail:
      ...
      `an innocent server's work while the update-all runs on its real host — ` +
