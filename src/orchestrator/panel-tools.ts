...
export function buildPanelToolDefs(): PanelToolDef[] {
  ...
  return [
    ...
    def(
      ...
      "WHY IS THAT NODE RED? Returns every errored node on the live canvas JOINED TO ITS CAUSE, which LiteGraph itself does not show — a red outline is just a boolean, so users legitimately see \"red node, no error message\". Prefer this over panel_get_errors when the user points at a node or asks WHY: that tool returns ComfyUI's raw error payloads, while this one attaches each cause to the actual node (type, title, widgets, wiring) and adds the two things the raw payload lacks — the MISSING-MODEL cause (exact file, its models directory, the widget holding it, and a download URL when known) and `red_outline` (whether that node is the one actually painted red). Each node carries `reasons[]`: `missing_model`, `validation` (per-input errors from the last queue attempt — message, details, offending input), and `execution` (runtime failure, with `exception_type` such as PIL.UnidentifiedImageError). NOTE: a node that throws AT RUNTIME is NOT painted red by LiteGraph, so it appears here with red_outline:false — reason enough to call this rather than eyeballing the canvas. Also returns graph-level `missing_models`, `missing_node_types` (or `missing_node_count` when the names can't be resolved), and `last_execution_error`. Call it whenever the user mentions a red/highlighted/erroring node, a failed run, or \"required models are missing\" — instead of guessing from widget values. A node flagged with no explaining source is reported too (it may be stale — re-running refreshes it). Read-only.",
      {},
