...
describe("#2693 a relaunch whose stdout is a file gets UTF-8", () => {
  ...
  it("honours an explicit value spelled in ANOTHER CASE (Windows env keys are case-insensitive)", () => {
    // The Copilot review on this PR: `({...process.env}).PYTHONUTF8` is undefined when the
    // inherited key is `PythonUtf8`, so a case-sensitive lookup would call it unset and add a
    // competing uppercase key. The explicit value must win, and no duplicate key may appear.
    const out = withUtf8StdioEnv({ PythonUtf8: "0", pythonioencoding: "cp932" }, WIN);
    expect(out.PythonUtf8).toBe("0");
    expect(out.pythonioencoding).toBe("cp932");
    expect(out).not.toHaveProperty("PYTHONUTF8");
    expect(out).not.toHaveProperty("PYTHONIOENCODING");
  });
