// Unit tests for the Qwen Code CLI ACP backend (qwen-backend.ts, issue #1417).
//
// We CANNOT run the real `qwen` CLI here, so we drive the REAL QwenBackend /
// AcpClient code end-to-end against a FAKE ACP server spoken over a mocked
// child_process — the same harness as gemini-backend.test.ts (Qwen Code's ACP
// surface is the same protocol). `node:child_process` is mocked so `spawn()`
// returns an in-process fake child whose stdin/stdout are wired to a tiny
// JSON-RPC-2.0 ACP server that implements the handshake (initialize →
// session/new), one streamed prompt turn (agent_thought_chunk + tool_call +
// tool_call_update + agent_message_chunk, then a session/prompt response with
// stopReason), and session/cancel. This exercises the line framing, the
// request/response correlation, the session/update → AgentEvent mapping, the
// terminal-result invariant, and interrupt — all deterministically, with no
// real binary.
        ...
        if (method === "initialize" && id !== undefined) {
          write({
            ...
            result: {
              ...
              agentInfo: { name: "qwen", title: "Qwen Code", version: "test" },
...
let QwenBackend: typeof import("../../orchestrator/qwen-backend.js").QwenBackend;

const QWEN_ENV_KEYS = ["DASHSCOPE_API_KEY", "BAILIAN_CODING_PLAN_API_KEY", "OPENAI_API_KEY"] as const;
...
beforeEach(async () => {
  ...
  for (const k of QWEN_ENV_KEYS) delete process.env[k];
  ({ QwenBackend } = await import("../../orchestrator/qwen-backend.js"));
});
...
describe("QwenBackend (ACP over stdio)", () => {
  it("spawns `qwen --acp` WITHOUT tool-only secrets in its env, but WITH its own DASHSCOPE_API_KEY", async () => {
    // SECURITY (PR #251): tool secrets (RunPod/CivitAI/HF…) live in process.env
    // for the comfyui tool child — they must NOT leak into the Qwen CLI's
    // spawn env. DASHSCOPE_API_KEY is the provider's OWN credential and must
    // stay (it isn't a tool-only key, so it passes through buildAgentSpawnEnv).
    ...
    process.env.DASHSCOPE_API_KEY = "sk-own-key";
    try {
      const backend = new QwenBackend({ cwd: process.cwd() });
      ...
      expect(env.DASHSCOPE_API_KEY).toBe("sk-own-key");
      // The spawn targets the qwen CLI in ACP mode (model pinned at spawn).
      expect(hoisted.spawnArgs[0]![0]).toBe("--acp");
    } finally {
  ...
  it("handshake → prompt → streamed updates → result maps to the canonical AgentEvent sequence", async () => {
    const backend = new QwenBackend({ cwd: process.cwd(), systemAppend: "BE NICE." });
  ...
  it("on auth_required, authenticates with the OPENAI method when DASHSCOPE_API_KEY is set (not method[0])", async () => {
    // The Qwen OAuth free tier was discontinued 2026-04-15; the fix must pick the
    // openai/api-key method, not blindly authMethods[0] (the dead OAuth one).
    process.env.DASHSCOPE_API_KEY = "sk-test-key";
    hoisted.config.requireAuth = true;
    hoisted.config.authMethods = [
      { id: "qwen-oauth", name: "Qwen OAuth" },
      { id: "openai", name: "OpenAI-compatible API key" },
    ];
    ...
    // It authenticated with the openai method id, then completed a normal turn.
    expect(hoisted.config.lastAuthMethodId).toBe("openai");
    expect(events.some((e) => e.type === "result" && e.ok === true)).toBe(true);

    await backend.close();
  });

  it("on auth_required, falls back to the first advertised method when no env key is set", async () => {
    hoisted.config.requireAuth = true;
    hoisted.config.authMethods = [
      { id: "qwen-oauth", name: "Qwen OAuth" },
      { id: "openai", name: "OpenAI-compatible API key" },
    ...
    expect(hoisted.config.lastAuthMethodId).toBe("qwen-oauth");
  ...
  it("listModels returns the static Qwen catalog (no effort metadata)", async () => {
    ...
    // Qwen has no discrete effort scale → no effort metadata (panel hides picker).
  ...
  it("applies the panel-selected model to the FIRST spawn (--model) before prepare", async () => {
    // No construction-time model; the panel-selected Qwen model arrives as
    // opts.model and must pin the very first `qwen --acp` spawn (P1a).
    const backend = new QwenBackend({ cwd: process.cwd() });
    const channel = makeChannel();
    const events: AgentEvent[] = [];
    const run = consume(backend.run({ channel: channel.iterable, model: "qwen3-coder-next" }), events);
    ...
    expect(hoisted.spawnArgs[0][hoisted.spawnArgs[0].indexOf("--model") + 1]).toBe("qwen3-coder-next");

    await backend.close();
  });

  it("a live setModel() respawns the CLI with the new --model and a fresh session", async () => {
    const backend = new QwenBackend({ cwd: process.cwd(), model: "qwen3-coder-plus" });
    const channel = makeChannel();
    const events: AgentEvent[] = [];
    const run = consume(backend.run({ channel: channel.iterable }), events);
    ...
    expect(hoisted.spawnArgs).toHaveLength(1);
    expect(hoisted.spawnArgs[0][hoisted.spawnArgs[0].indexOf("--model") + 1]).toBe("qwen3-coder-plus");

    // Live model switch (panel picker → PanelAgent.setOptions → agent.setModel).
    await backend.setModel("qwen3.7-plus");
    ...
    // A SECOND `qwen --acp` was spawned, pinned to the new model.
    expect(hoisted.spawnArgs).toHaveLength(2);
    expect(hoisted.spawnArgs[1]).toContain("--acp");
    expect(hoisted.spawnArgs[1][hoisted.spawnArgs[1].indexOf("--model") + 1]).toBe("qwen3.7-plus");
  ...
  it("ignores a non-Qwen model id passed to setModel (e.g. the Claude panel model)", async () => {
    const backend = new QwenBackend({ cwd: process.cwd(), model: "qwen3-coder-plus" });
    const channel = makeChannel();
    const events: AgentEvent[] = [];
    const run = consume(backend.run({ channel: channel.iterable }), events);
    ...
    // PanelAgent may relay the Claude panel model — it must NOT respawn Qwen.
...
describe("qwen buildAcpMcpServers", () => {
  it("maps http specs to the ACP SSE variant (live CLIs reject type 'http')", async () => {
    const { buildAcpMcpServers } = await import("../../orchestrator/qwen-backend.js");
