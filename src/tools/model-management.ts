...
export function registerModelManagementTools(server: McpServer): void {
  server.tool(
    ...
    "Find model weights and get them onto the connected ComfyUI, and track the transfers. Driven by the `action` parameter:\n" +
      ...
      '- action:"status" — Check on downloads started by action:"download" / action:"download_civitai". Reports each download\'s state (downloading / done / error / cancelled), its destination path once it lands, and byte progress when the panel progress channel is enabled. Use this after a download reports it is still running — that means the transfer is in flight, NOT that it failed. What survives a reconnect depends on WHICH reconnect, so do not read one answer for both. Across an AGENT/sidebar session reconnect the orchestrator is still up: the download is genuinely still streaming, reads `downloading`, and is resolvable by its `id` OR by `url` — confirm it rather than starting a duplicate. Across an ORCHESTRATOR RESTART the transfer is dead: it streamed inside the process that exited, so re-issuing IS the correct move. What you can see of it afterwards is weaker than that, in two ways worth knowing before you rely on it: the carried-over record is findable by `id` ONLY (not by `url` — the carry-over does not preserve the url lookup key), and it may not exist at all, because carrying it over is best-effort. So a MISSING record is not evidence the download is alive. When the record is there it is terminal (`error`) and carries an explicit INTERRUPTED note saying so. Omit `id` and `url` to list every tracked download. A previous session\'s download whose heartbeat has gone stale is reported with a stale-heartbeat NOTE: action:"cancel" can close it once the writer is proven gone, and re-issuing action:"download" then resumes or restarts it. Read-only.\n' +
