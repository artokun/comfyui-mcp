...
export function registerModelManagementTools(server: McpServer): void {
  server.tool(
    ...
    "Find model weights and get them onto the connected ComfyUI, and track the transfers. Driven by the `action` parameter:\n" +
      ...
      '- action:"status" — Check on downloads started by action:"download" / action:"download_civitai". Reports each download\'s state (downloading / done / error / cancelled), its destination path once it lands, and byte progress when the panel progress channel is enabled. Use this after a download reports it is still running — that means the transfer is in flight, NOT that it failed. Across an AGENT/sidebar session reconnect a download this MCP streams locally keeps running and is normally resolvable by `id` or by `url`. An ORCHESTRATOR RESTART is different and its report is WEAKER THAN IT SOUNDS: any record carried across one reads terminal (`error`) with an INTERRUPTED note, and is findable by `id` ONLY, never by `url` (the carry-over drops the url lookup key). Treat that note as "this MCP stopped watching", NOT as "the bytes stopped" — it is written without checking whether the writer is still alive, and for a download dispatched to ComfyUI-Manager the fetch runs on the ComfyUI HOST, which a restart here does not touch at all. So do NOT re-issue on the strength of an INTERRUPTED note alone, above all for a Manager dispatch: re-issuing a live server-side fetch writes a SECOND copy to the same destination and corrupts the model. Wait and re-check list_local_models on the connected server until the file appears and its size stops changing (an in-progress file is not listed, and Manager may stage it under its own type dir, so an empty folder mid-download is normal). And NOT FOUND NEVER MEANS STOPPED — both the cross-session record and the carry-over are written best-effort, so their absence is evidence of nothing. Omit `id` and `url` to list every tracked download. A previous session\'s download whose heartbeat has gone stale is reported with a stale-heartbeat NOTE: action:"cancel" can close it once the writer is proven gone, and re-issuing action:"download" then resumes or restarts it. Read-only.\n' +
