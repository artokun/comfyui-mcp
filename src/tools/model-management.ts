...
export function registerModelManagementTools(server: McpServer): void {
  server.tool(
    ...
    "Find model weights and get them onto the connected ComfyUI, and track the transfers. Driven by the `action` parameter:\n" +
      ...
      '- action:"status" — Check on downloads started by action:"download" / action:"download_civitai". Reports each download\'s state (downloading / done / error / cancelled), its destination path once it lands, and byte progress when the panel progress channel is enabled. Use this after a download reports it is still running — that means the transfer is in flight, NOT that it failed. Findable across a reconnect — but READ THE STATE rather than assuming the transfer survived. A download started in a previous session is still resolvable by its `id` (or by `url`) in both of these cases, which are otherwise indistinguishable from your side: across an AGENT/sidebar session reconnect it is genuinely still streaming and reads `downloading`, so confirm it rather than starting a duplicate; across an ORCHESTRATOR RESTART it is NOT running — the transfer streamed inside the process that exited — and the record comes back terminal (`error`, INTERRUPTED), where re-issuing the download IS the correct move. The state is the only thing that tells them apart. Omit `id` and `url` to list every tracked download. A previous session\'s download whose heartbeat has gone stale is reported with a stale-heartbeat NOTE: action:"cancel" can close it once the writer is proven gone, and re-issuing action:"download" then resumes or restarts it. Read-only.\n' +
            ...
            content: [
              {
                ...
                text: (args.id || args.url)
                  ? `No download matching ${selector}. Three ways to get here and this lookup does not distinguish them: it finished long ago (settled records are pruned after a while), it never started, or it was interrupted by an orchestrator restart and the carry-over that records that (#1148) did not run — that migration is best-effort by design, so its absence is not evidence the download is alive. Check the panel download tray before re-downloading. Within the SAME session, re-issuing an identical in-flight download adopts it rather than duplicating; across a reconnect, confirm via the tray first.`
