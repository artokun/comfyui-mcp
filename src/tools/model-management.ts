...
export function registerModelManagementTools(server: McpServer): void {
  server.tool(
    ...
    "Find model weights and get them onto the connected ComfyUI, and track the transfers. Driven by the `action` parameter:\n" +
      ...
      '- action:"status" — Check on downloads started by action:"download" / action:"download_civitai". Reports each download\'s state (downloading / done / error / cancelled), its destination path once it lands, and byte progress when the panel progress channel is enabled. Use this after a download reports it is still running — that means the transfer is in flight, NOT that it failed. What survives depends on WHICH reconnect and on WHO is doing the transferring, so do not read one answer for all of them. Across an AGENT/sidebar session reconnect the orchestrator is still up and a download it streams locally keeps running; it is normally resolvable by `id` or by `url`, but cross-session resolution reads a record persisted best-effort, so a lookup finding nothing has NOT established the transfer stopped. Across an ORCHESTRATOR RESTART a download this MCP was streaming LOCALLY is dead — it streamed inside the process that exited — and re-issuing is correct; when a record for it is carried over it is terminal (`error`) with an explicit INTERRUPTED note, findable by `id` ONLY (not by `url` — the carry-over does not preserve the url lookup key), and that carry-over is best-effort too, so its absence is evidence of nothing either way. NONE of that applies to a download dispatched to ComfyUI-Manager: that is committed done the moment the dispatch is ACCEPTED, and with an aria2 sidecar the host can still be streaming gigabytes long afterwards — so a restart does not kill it and does not produce an interrupted record for it. Do NOT re-issue a Manager download on the strength of a restart; confirm with list_local_models first, or you will start a second copy of a transfer that is still running. Omit `id` and `url` to list every tracked download. A previous session\'s download whose heartbeat has gone stale is reported with a stale-heartbeat NOTE: action:"cancel" can close it once the writer is proven gone, and re-issuing action:"download" then resumes or restarts it. Read-only.\n' +
