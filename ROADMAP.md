...
`comfyui-mcp` is compatible with both **Claude Code** and **Google Antigravity** agents.

### Google Antigravity Setup
The Antigravity assets are dynamically generated during installation.
1. Clone this repository and run `npm install`. The `sync-agents` script will create the necessary `.agents/` and `.gemini/` directories.
2. Ensure you have the `comfyui-mcp` workspace loaded in your Antigravity environment.
3. The server will use the automatically generated `.agents/mcp_config.json`.
See [GEMINI.md](./GEMINI.md) for development notes.

### Claude Code Setup
1. Use the marketplace: `/plugin marketplace add artokun/comfyui-mcp` followed by `/plugin install comfy`.
2. Or use the local plugin folder: `claude --plugin-dir ./plugin`
...
### 0.6.1 – 0.6.0 — 2026-05-25

- **0.6.1** — `upload_video`/`upload_audio` tools.
- **0.6.0** — comfy-cli capability port (custom-node management, snapshots, bisect, workflow dependencies, install/update ComfyUI, models, workspace/env, API nodes, manager config).
...
## Theme F — Cross-Agent Ecosystem (NEW)
- **F1 — Native Antigravity Panel:** Adapt the ComfyUI agent panel to work natively within Antigravity workspaces.
- **F2 — Unified Agent Packaging:** Standardize the skill/command translation logic to publish universally across Claude, Antigravity, and other agents.