...
<ParamField path="auth" type="object">
  action:"download" / action:"download_civitai" — optional per-request authentication for private/gated model URLs. When provided it overrides built-in HuggingFace/CivitAI token handling.
</ParamField>
...
<ParamField path="model_root" type="string">
  action:"download" / action:"download_civitai" — optional absolute model-root directory already listed by list_local_models action:"list_paths" (or the resolver's known extra/primary roots). Use this when the live ComfyUI is unreachable so the file lands in a configured extra-model root instead of &lt;COMFYUI_PATH&gt;/models. Invented paths are refused.
</ParamField>
