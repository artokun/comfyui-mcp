...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  جذر اختياري لمستودع الكود في التثبيتات المنقسمة، حيث تكون `main.py` و`.venv`
  منفصلة عن بيانات المستخدم والنماذج. يستخدمه pip وvenv وتحديث النواة.
  تبقى قراءة/كتابة الحزم (`custom_nodes`) على `--base-directory` / `COMFYUI_PATH`.
  يُتجاهل في الوضع البعيد.
</ParamField>
...
<ParamField path="COMFYUI_DOWNLOAD_CONNECTIONS" type="number" default="4">
  عدد الاتصالات المتوازية لكل عملية تنزيل نموذج. الاتصال الواحد محدود بزمن الذهاب والإياب، وفي كثير من شبكات توزيع النماذج بحدّ سرعة لكل اتصال؛ جلب نطاقات البايت بالتوازي يتجاوز الاثنين. القيمة `1` تعطّل ذلك وتنزّل عبر اتصال واحد. لا يُستخدم إلا عندما يعلن المضيف دعم نطاقات البايت ويثبته في طلب اختبار — وإلا يجري التنزيل عبر اتصال واحد تمامًا كما كان.
</ParamField>
<ParamField path="COMFYUI_DOWNLOAD_SEGMENT_MIN_MB" type="number" default="64">
  الحد الأدنى لحجم الملف بالميغابايت الذي يستحق التقسيم على عدة اتصالات. دون ذلك تكلّف الرحلات الإضافية أكثر مما تعيده الموازاة.
</ParamField>
