...
- **Atlas Cloud** — set `ATLASCLOUD_API_KEY` from the
  [Atlas Cloud console](https://www.atlascloud.ai/console/api-keys). The default
  model is `qwen/qwen3.5-flash` over the OpenAI-compatible
  `https://api.atlascloud.ai/v1` endpoint. Override the model with
  `COMFYUI_MCP_ATLASCLOUD_MODEL` or the base URL with
  `COMFYUI_MCP_ATLASCLOUD_BASE_URL`.
...
Blind's enforcement reaches the agent's **native tools** too, not just the
comfyui MCP surface: the built-in Claude backend runs with a PreToolUse gate
that denies its own `Read`/`WebFetch` on image content (raster files by
