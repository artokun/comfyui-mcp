...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  Raíz opcional del checkout de código para instalaciones divididas, donde `main.py`
  y `.venv` están separados de los datos y modelos. Pip, venv y las actualizaciones
  del núcleo usan este checkout. Las lecturas/escrituras de packs (`custom_nodes`)
  permanecen en `--base-directory` / `COMFYUI_PATH`. Se ignora en modo remoto.
</ParamField>
<ParamField path="COMFYUI_RESTART_COMMAND" type="string">
  Comando de shell que reinicia un ComfyUI **gestionado externamente** — p. ej.
  `docker restart comfyui` o `systemctl --user restart comfyui`. Cuando está
  definido, `restart_comfyui` ejecuta este comando en lugar de kill+relaunch
  (que exige que la ruta de arranque de la instalación sea resoluble —
  imposible para un contenedor o un lanzador cuyo `main.py` solo se ancla
  dentro de su propio espacio de nombres), y `panel_restart_comfyui` también
  pasa por él. Solo objetivos locales; los objetivos remotos/nube conservan la
  vía de reinicio por Manager. El reinicio verifica que la instancia volvió
  sana y solo reporta el ciclo como confirmado cuando el down→up se observó
  realmente.
</ParamField>
...
<ParamField path="COMFYUI_DOWNLOAD_CONNECTIONS" type="number" default="4">
  Conexiones paralelas por descarga de modelo. Una sola conexión está limitada por el tiempo de ida y vuelta y, en muchas CDN de modelos, por un límite de velocidad por conexión; descargar rangos de bytes en paralelo supera ambos. `1` lo desactiva y descarga por una sola conexión. Solo se usa cuando el servidor anuncia soporte de rangos de bytes y lo demuestra en una petición de sondeo; si no, la descarga se hace por una sola conexión exactamente como antes.
</ParamField>
<ParamField path="COMFYUI_DOWNLOAD_SEGMENT_MIN_MB" type="number" default="64">
  Tamaño mínimo de archivo, en MB, que merece repartirse entre varias conexiones. Por debajo, las peticiones adicionales cuestan más de lo que aporta el paralelismo.
</ParamField>
...
<ParamField path="COMFYUI_MCP_BRIDGE_PORT" type="number" default="9199">
  ...
  **orquestador del panel** (por defecto **9199**).
</ParamField>
