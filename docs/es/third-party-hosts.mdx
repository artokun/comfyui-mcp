...
(`COMFYUI_MCP_BRIDGE_PORT`, por defecto **9199**):

| Puerto | Propósito |
|---|---|
| `bridge` (9199) | El puente UI panel ⇄ orquestador (la propia conexión del escritorio). |
| `bridge − 1` (9198) | La superficie HTTP-MCP `panel_*` — **no** este protocolo. |
| **`bridge − 2` (9197)** | **El listener de emparejamiento / control remoto al que te conectas.** Filtrado por token, enlazado en `0.0.0.0` (alcanzable en la LAN). |
