...
**Aviso de visión: hoy estas etiquetas son solo texto.** La exportación GGUF
actual perdió el proyector multimodal de Gemma 4
([#642](https://github.com/artokun/comfyui-mcp/issues/642)), así que el
driver por defecto no puede *ver*: los bucles de crítica de
`get_image (action:"view")`, `get_workflow (action:"from_image")` y las
comparaciones visuales no están disponibles — exactamente la fila
**Visión** «vuela a ciegas» de la tabla de capacidades de arriba. Las
llamadas a herramientas, la generación y la gestión del grafo no se ven
afectadas. Si el bucle de visión te importa hoy, elige un modelo con visión
según «Requisitos del modelo» en lugar del predeterminado.
