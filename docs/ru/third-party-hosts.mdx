...
| Порт | Назначение |
|---|---|
| `bridge` (9199) | UI-мост панель ⇄ оркестратор (собственное соединение рабочего стола). |
| `bridge − 1` (9198) | HTTP-MCP поверхность `panel_*` — **не** этот протокол. |
| **`bridge − 2` (9197)** | **Слушатель сопряжения / удалённого управления, к которому вы подключаетесь.** С гейтингом по токену, привязан к `0.0.0.0` (доступен из LAN). |
