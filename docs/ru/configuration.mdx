...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  Необязательный корень checkout кода для раздельных установок, где `main.py` и
  `.venv` находятся отдельно от данных и моделей. Pip, venv и обновление ядра
  используют этот путь. Чтение/запись пакетов (`custom_nodes`) остаётся на
  `--base-directory` / `COMFYUI_PATH`. В удалённом режиме путь игнорируется.
</ParamField>
...
<ParamField path="COMFYUI_DOWNLOAD_CONNECTIONS" type="number" default="4">
  Число параллельных соединений на одну загрузку модели. Одно соединение ограничено временем кругового обхода, а на многих CDN для моделей — ещё и лимитом скорости на соединение; параллельная загрузка байтовых диапазонов снимает оба ограничения. `1` отключает механизм и загружает в одно соединение. Применяется, только если хост объявляет поддержку байтовых диапазонов и подтверждает её пробным запросом, — иначе загрузка идёт в одно соединение ровно как раньше.
</ParamField>
<ParamField path="COMFYUI_DOWNLOAD_SEGMENT_MIN_MB" type="number" default="64">
  Минимальный размер файла в МБ, который имеет смысл делить между несколькими соединениями. Ниже этого порога дополнительные обращения обходятся дороже выигрыша от параллелизма.
</ParamField>
