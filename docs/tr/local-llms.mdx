...
**Görüş uyarısı: bu etiketler bugün yalnızca metin.** Mevcut GGUF dışa
aktarımı Gemma 4'ün çok kipli projektörünü düşürdü
([#642](https://github.com/artokun/comfyui-mcp/issues/642)); bu yüzden
varsayılan sürücü *göremez*: `get_image (action:"view")` eleştiri döngüleri,
`get_workflow (action:"from_image")` ve görsel karşılaştırmalar kullanılamaz —
tam olarak yukarıdaki yetenek tablosunun **Görüş** "kör uçar" satırı. Araç
çağırma, üretim ve grafik yönetimi etkilenmez. Görüş döngüsü bugün sizin için
önemliyse, varsayılan etiket yerine "Model gereksinimleri" uyarınca görüş
yeteneği olan bir model seçin.
