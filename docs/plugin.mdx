...
description: "comfyui-mcp is a full Claude Code plugin: 42 AI skills, 11 slash commands, 4 autonomous agents, and 3 hooks on top of the 38 MCP tools — expert ComfyUI knowledge that grows with every release."
icon: "puzzle-piece"
---
...
| `lora-manager` | ComfyUI-LoRA-Manager from the panel — add `LoRA Text Loader (LoraManager)` (STRING `lora_syntax`); `Lora Loader (LoraManager)` and other AUTOCOMPLETE_TEXT_* nodes wait 5s then refuse even when the pack is healthy |
