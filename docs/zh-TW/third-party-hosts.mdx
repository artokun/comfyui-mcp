...
配對監聽器從橋接連接埠派生（`COMFYUI_MCP_BRIDGE_PORT`，預設 **9199**）：

| 連接埠 | 用途 |
|---|---|
| `bridge`（9199） | 面板 ⇄ 協調器 UI 橋接（桌面自己的連線）。 |
| `bridge − 1`（9198） | `panel_*` HTTP-MCP 面 —— **不是**本協定。 |
| **`bridge − 2`（9197）** | **你要連的配對 / 遠端控制監聽器。** 權杖把門，綁在 `0.0.0.0`（區域網可達）。 |
