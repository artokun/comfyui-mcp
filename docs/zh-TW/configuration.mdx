...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  用於分離式安裝的可選程式碼 checkout 根目錄；此時 `main.py` 與 `.venv` 和資料及模型
  分開存放。pip / venv / 核心更新使用此路徑。節點套件讀寫（`custom_nodes`）仍落在
  `--base-directory` / `COMFYUI_PATH`。遠端模式不會用它修改本機檔案系統。
</ParamField>
...
<ParamField path="COMFYUI_DOWNLOAD_CONNECTIONS" type="number" default="4">
  每個模型下載使用的平行連線數。單一連線受往返時延限制，在許多模型 CDN 上還受每連線限速；平行抓取位元組範圍可突破這兩者。設為 `1` 會關閉此機制，改用單一連線下載。僅在檔案主機宣告支援位元組範圍、並在探測請求中證實時才啟用——否則下載與先前完全一樣走單一連線。
</ParamField>
<ParamField path="COMFYUI_DOWNLOAD_SEGMENT_MIN_MB" type="number" default="64">
  值得拆分到多條連線的最小檔案大小（MB）。低於此值時，額外往返的成本會超過平行化帶來的收益。
</ParamField>
...
<ParamField path="COMFYUI_MCP_BRIDGE_PORT" type="number" default="9199">
  **面板協調器**擁有的面板 WebSocket 橋接的回送連接埠（預設 **9199**）。
</ParamField>
