...
**視覺提醒：這些標籤目前只有文字能力。** 目前的 GGUF 匯出掉了 Gemma 4 的多模態
投影器（[#642](https://github.com/artokun/comfyui-mcp/issues/642)），所以預設驅動
看不見 —— `get_image (action:"view")` 批評迴圈、
`get_workflow (action:"from_image")` 以及視覺對比都不可用，正是上面能力表裡
**視覺** 那一列的「盲飛」。工具呼叫、生成與圖管理都不受影響。如果你現在就需要視覺
迴路，請按「模型需求」挑一個支援視覺的模型，而不是用預設標籤。
