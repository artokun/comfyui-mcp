...
(`COMFYUI_MCP_BRIDGE_PORT`, padrão **9199**):

| Porta | Propósito |
|---|---|
| `bridge` (9199) | A ponte de UI painel ⇄ orquestrador (a conexão própria do desktop). |
| `bridge − 1` (9198) | A superfície HTTP-MCP `panel_*` — **não** este protocolo. |
| **`bridge − 2` (9197)** | **O listener de pareamento / controle remoto ao qual você se conecta.** Protegido por token, bind em `0.0.0.0` (alcançável na LAN). |
