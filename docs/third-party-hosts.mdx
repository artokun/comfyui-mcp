...
| Port | Purpose |
|---|---|
| `bridge` (9199) | The panel ⇄ orchestrator UI bridge (the desktop's own connection). |
| `bridge − 1` (9198) | The `panel_*` HTTP-MCP surface — **not** this protocol. |
| **`bridge − 2` (9197)** | **The pairing / remote-control listener you connect to.** Token-gated, bound on `0.0.0.0` (LAN-reachable). |
...
9180-era URLs (bridge **+ 2**, default pairing **9182**) still pair: a session that
was already on 9180 keeps that block across self-restart, so a phone that saved
`ws://…:9182/?token=…` does not need to re-pair.
