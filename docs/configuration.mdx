...
<ParamField path="COMFYUI_PATH" type="string">
  Absolute path to the local ComfyUI data/base directory. In a conventional install this is also
  the code checkout. Auto-detected from common locations when unset (suppressed in remote/cloud
  modes). Component-aware tools prefer the running server's explicit input/output/models paths.
</ParamField>
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  Optional code-checkout root for split installs where `main.py` and `.venv` are separate
  from the data/model/user root. Pip, venv, and core updates use this checkout (core updates
  prefer the checkout observed from the running local server). Pack reads/writes —
  `custom_nodes`, comfy-cli `--workspace`, `apply_manifest` clone/checkout, workflow-lock
  pack commits — stay on the live `--base-directory` / `COMFYUI_PATH` data root (#1770).
  Ignored for local filesystem mutation in remote mode.
</ParamField>
<ParamField path="COMFYUI_RESTART_COMMAND" type="string">
  Shell command that restarts an **externally managed** ComfyUI — e.g. `docker restart comfyui`
  or `systemctl --user restart comfyui`. When set, `restart_comfyui` runs this command instead
  of kill+relaunch (which needs the install's launch path resolvable — impossible for a
  container or launcher whose `main.py` anchors only inside its own namespace), and
  `panel_restart_comfyui` routes through it too. Local targets only; remote/cloud targets keep
  the Manager-reboot path. The restart verifies the instance came back healthy and reports the
  cycle as confirmed only when the down→up was actually observed.
</ParamField>
...
<ParamField path="COMFYUI_DOWNLOAD_CONNECTIONS" type="number" default="4">
  Parallel connections per model download. A single connection is capped by round-trip time and, on many model CDNs, by a per-connection rate limit; fetching byte ranges in parallel lifts both. `1` disables it and downloads over one connection. Used only when the file host advertises byte-range support and proves it on a probe request — otherwise the download runs over a single connection exactly as before.
</ParamField>
<ParamField path="COMFYUI_DOWNLOAD_SEGMENT_MIN_MB" type="number" default="64">
  Minimum file size, in MB, worth splitting across multiple connections. Below it the extra round-trips cost more than the parallelism returns.
</ParamField>
...
<ParamField path="COMFYUI_MCP_BRIDGE_PORT" type="number" default="9199">
  ...
  owns (default **9199**; was 9180 — that port collides with Logitech G HUB on
  many desktops. A live 9180 session is not moved; set this to pin).
</ParamField>
