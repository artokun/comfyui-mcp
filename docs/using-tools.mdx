...
Long reads are capped so that one enormous graph cannot swallow the whole conversation. When a read is trimmed you get a note saying so, and it suggests an argument to raise.

**Check that suggestion against what actually ran out**, because two different ceilings can stop the same read and the suggestion does not always name the right one:

- The note reads `truncated at 40 of 300` — the first number is exactly the node limit in force (40 unless someone raised it), so `limit` is what ran out.
- It stops at some arbitrary-looking number instead, usually on rows carrying a lot of detail — then it is `max_chars`, the character budget.

Raising the one that was not the problem changes nothing, which reads exactly like the retry having failed. The agent is supposed to work this out and retry by itself; when it doesn't, you are the fallback, and this is the sentence:

> That got truncated — retry the same query, raising whichever limit it actually hit.
...
The message splits these into two groups for you — it distinguishes "connected earlier and dropped" from "nothing has connected yet" — and the first of those is always just a reload. It does not go further than that, and says so: within the second group it walks you through the checks above in order rather than naming a cause it has no way to observe.
...
<Steps>
  <Step title="Ask what it can see">
    ...
    **absent**: the tools never reached your client. That is something on your
    client's side rather than a panel or server fault, and it splits again into
    two — never wired up, or a host that cannot hold them at all. The list
