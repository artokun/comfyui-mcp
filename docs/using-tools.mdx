...
**Check that suggestion against what actually ran out.** Two different ceilings can stop the same read, and the suggestion does not always name the one that fired.

The note reads like `truncated at 40 of 300`. Compare that first number with the node limit that was in force — 40 unless the agent had already raised it:

- **They match.** `limit` ran out: it listed as many nodes as it was allowed to.
- **It stopped short of the limit**, usually on rows carrying a lot of detail. Then it was `max_chars`, the character budget.

Raising the one that was not the problem changes nothing, which reads exactly like the retry having failed. And if the argument is already at its ceiling, raising it is not available at all — narrow the question instead, as below. The agent is supposed to work this out and retry by itself; when it doesn't, you are the fallback, and this is the sentence:
...
<Steps>
  <Step title="Ask what it can see">
    ...
    **No names at all**, or "I don't have any tools for ComfyUI", rules out the
    ...
    One check separates them, and it is not something the agent can see: **open your
    ...
    - **It is there and connected, but the model still lists no tools** → they are
      being withheld between the two. That is **blocked**, and the same list below
      says where to look.
  </Step>
  <Step title="Ask it to try, and to report back verbatim">
    ...
    **Don't work around it.** The whole failure mode is an agent quietly routing
    ...
    - **"I don't have that tool"** — absent *or* blocked, still. A withheld tool and
      a missing one look the same from the model's seat, so settle it with the
      server-list check in step 1 rather than acting on this.
