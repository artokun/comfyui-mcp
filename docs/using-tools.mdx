...
Long reads are capped so that one enormous graph cannot swallow the whole conversation. When a read is trimmed you get a note saying so, and **that note names the argument that ran out** — which is not always the same one. Two different ceilings can stop a read: the character budget (`max_chars`) or the number of nodes listed (`limit`). Raising the wrong one changes nothing and looks like the retry failed.
...
> That got truncated — retry the same query, raising whichever limit the note said you hit.
