...
Long reads are capped so that one enormous graph cannot swallow the whole conversation. When a read is trimmed you get a note saying so, and it names an argument to raise.

**Check which argument.** Two different ceilings can stop the same read — the character budget (`max_chars`) or the number of nodes listed (`limit`) — and raising the one that was not the problem changes nothing, which reads exactly like the retry having failed.

The agent is supposed to read its own truncation note and retry by itself. When it doesn't, you are the fallback, and this is the sentence:
