...
| `panel_view_errored_nodes` | **Why** a node is red — each cause joined to the node (missing model + download URL, validation, runtime `exception_type`) |
| `panel_get_errors` | ComfyUI's raw validation-error map + last execution error |
